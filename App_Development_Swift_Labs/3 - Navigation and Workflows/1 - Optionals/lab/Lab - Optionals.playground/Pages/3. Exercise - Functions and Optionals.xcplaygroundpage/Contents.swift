...
func checkAge (age: String) -> Int? {
    if let theAge = Int(age) {
        ...
        return nil // returning nil at if let else
    }
}
...
print(makePurchase(itemName: "Banana"))
print(makePurchase(itemName: "Donuts"))
print(makePurchase(itemName: "incorrect"))
...
/* First solution
func makePurchase (itemName: String) -> Double? {
    let itemNameInput = itemName.self
    let itemPrice : Double = prices["\(itemNameInput)"]!
    
    if let inStock = stock["\(itemNameInput)"] {
        if inStock > 0 {
            print (itemPrice)
    return itemPrice
            } // end if inStock
