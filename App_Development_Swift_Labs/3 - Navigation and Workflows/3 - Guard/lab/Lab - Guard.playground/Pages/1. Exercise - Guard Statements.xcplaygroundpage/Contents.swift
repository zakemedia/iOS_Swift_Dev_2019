...
func createUser() -> User? {
    guard let unwrappedFirstName = firstNameTextField.text,
        let unwrappedLastName = lastNameTextField.text,
        let unwrappedAge = ageTextField.text else { return nil}

        let myUser = User(firstName: unwrappedFirstName, lastName: unwrappedLastName, age: unwrappedAge)
    return myUser
}
/*:
...
let newUser = createUser()
    if let newUser = newUser { // unwraps
        print("\(newUser.firstName) \(newUser.lastName) is \(newUser.age) old") //using properties
}
// WJ Check this
